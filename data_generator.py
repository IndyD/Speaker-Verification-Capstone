...
def pre_proccessing(audio, rate, pre_emphasis = 0.97, frame_size=0.02, frame_stride=0.01):
	...
	# Pad audio to make sure that all frames have equal number of samples 
	# without truncating any samples from the original audio
...
def convert_to(audio1, audio2, labels, tf_record_path):
...
def extra_params(section, step_tf):
	...
	name_base = section + '_' + str(datetime.datetime.now())
	total_files = int(NUM_FILES[section] * ROUNDS[section]/step_tf)
	input_file	= open(FILES[section],'r')
	num_total_files_per_round = NUM_FILES[section]
	if DATA_VAD_START[section] != "":
		data_start = np.load(DATA_VAD_START[section])
		data_end = np.load(DATA_VAD_END[section])

	return name_base, total_files, input_file, num_total_files_per_round, data_start, data_end	
...
def create_data_array(matrix, row_matrix, data_start, data_end):
	...
	while len(X1)< N_ROW_TF_RECORD:
		...
		if start.shape[0]>0:		
			...
			list_same_class = []
			for i_same_class in range(0,len(matrix)):
...
FILES['train'] = PARAMS.PATHS.file_train
FILES['validation'] = PARAMS.PATHS.file_valid
FILES['test'] = PARAMS.PATHS.file_test
DATA_VAD_START['train'] = PARAMS.PATHS.data_vad_train_start
DATA_VAD_START['validation'] = PARAMS.PATHS.data_vad_valid_start
DATA_VAD_START['test'] = PARAMS.PATHS.data_vad_test_start
DATA_VAD_END['train'] = PARAMS.PATHS.data_vad_train_end
DATA_VAD_END['validation'] = PARAMS.PATHS.data_vad_valid_end
DATA_VAD_END['test'] = PARAMS.PATHS.data_vad_test_end
ROUNDS['train'] = PARAMS.DATA_GENERATOR.rounds_train
ROUNDS['validation'] = PARAMS.DATA_GENERATOR.rounds_valid
ROUNDS['test'] = PARAMS.DATA_GENERATOR.rounds_test
NUM_FILES['train'] = PARAMS.DATA_GENERATOR.num_train_files
NUM_FILES['validation'] = PARAMS.DATA_GENERATOR.num_valid_files
NUM_FILES['test'] = PARAMS.DATA_GENERATOR.num_test_files
N_ROW_TF_RECORD = PARAMS.DATA_GENERATOR.n_row_tf_record
OUTPUT_DIR = PARAMS.DATA_GENERATOR.output_dir
...
#f os.path.exists(str(DIRECTORY_TEST)) == False:
#	os.mkdir(str(DIRECTORY_TEST))

# Compute for the desired number of epochs.
for n_epochs in range(NUM_EPOCHS):

	if n_epochs == (NUM_EPOCHS-1):
		sections = ['train', 'test', 'validation']
	else: 
		sections = ['train', 'test']

	for section in sections:

		name_base, total_files, input_file, num_total_files_per_round, data_start, data_end = extra_params(
			section, step_tf)		
		...
		for n_file in range(total_files):	
			...
			tf_record_path = os.path.join(OUTPUT_DIR, name + '.tfrecords')
			database = np.array(glob.glob( os.path.join(OUTPUT_DIR, '*.tfrecords') ))
			exists = np.where(database==tf_record_path)

			while len(exists[0]) > 0:
				database = np.array(glob.glob( os.path.join(OUTPUT_DIR, '*.tfrecords') ))
				exists = np.where(database==tf_record_path)

			convert_to(X1_array,X2_array,Y_array,tf_record_path)
